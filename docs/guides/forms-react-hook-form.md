...
이 프로젝트의 불변 규칙([CLAUDE.md](../../CLAUDE.md) 규칙 2)은 **비밀번호 최소 6자**이며,
...
| Server Action에서 `authenticateUser()` | `POST /api/auth/login` 호출 후 JWT 저장 |
...
// ⚠️ 불변 규칙(CLAUDE.md 규칙 2): 6자 이상이면 통과.
...
// POST /api/auth/login — 자격 검증과 JWT 발급은 백엔드 책임
...
export async function signup(body: {
  ...
  return apiClient.post<ApiResponse<unknown>>('/api/auth/signup', body)
}
```
...
export function LoginForm() {
  ...
  const mutation = useMutation({
    ...
    onError: (error: unknown) => {
      // 검증 실패(COMMON_001)면 data가 필드별 에러 맵으로 내려온다 (API_SPEC.md 1.3)
      ...
      // 자격 불일치(AUTH_001) 등은 폼 전체 에러로 표시
      form.setError('root', { message: '이메일 또는 비밀번호를 확인해주세요.' })
    },
  })
...
| 중복 이메일 검사 | 프론트의 `getUserByEmail()` | 백엔드 → `AUTH_002` 응답 |
| 세션 | 쿠키 세션 | JWT (24h) 클라이언트 저장 |
| 제출 트리거 | `useActionState` + `form action` | React Query `mutation` |
| 에러 표시 | Action 반환값 | `ApiResponse.errorCode` / 필드 에러 맵 |
