...
이 문서는 Claude Code에서 Next.js 16 프로젝트(현재 설치 버전: **16.3.1**)를 개발할 때 따라야 할 핵심 규칙과 가이드라인을 제공합니다.
...
`package.json`에서 `next`는 **`16.3.1`로 정확히 핀** 되어 있어 설치 시점과 무관하게 버전이 고정됩니다. `eslint-config-next`도 **동일한 `16.3.1`** 이므로 lint 규칙과 프레임워크 버전이 일치합니다.
