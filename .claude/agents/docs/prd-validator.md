...
3. **토큰 2종 구조**: Access Token(JWT, **30분**) + Refresh Token(불투명, **14일**, **회전**). Refresh는 **httpOnly + Secure + SameSite 쿠키로만** 전달하고 DB에 **SHA-256 해시로만** 저장한다. localStorage·응답 본문 저장 금지
4. **로그아웃은 서버 호출**로 Refresh Token 폐기 + 쿠키 만료. 클라이언트 삭제만으로 끝내지 않음
5. 삭제는 **Soft Delete**(`deleted_at`), 물리 삭제 금지
6. 목록 조회는 **페이지네이션 필수**
7. 리소스는 **본인 소유만 접근** (서버 소유권 검증). 위반 시 403이 아니라 **404** 응답
8. DB명 **`todolist_db`**, 테스트 DB `todolist_db_test` — 모두 소문자 고정
9. **비밀번호 재설정**: 1회용·**30분 만료**·DB에 해시 저장. 완료 시 **전체 Refresh Token 폐기**. 계정 유무와 무관하게 동일 응답(계정 열거 방지)
10. **소셜 로그인은 Google 전용** (백엔드 주도 리다이렉트). 동일 이메일은 기존 계정에 **연결**
11. **Tiptap 본문 이중 정화**: 서버 Jsoup sanitize + 클라이언트 DOMPurify. 하나만 하면 위반
12. **패키지명 `com.example.todoapp`**, 엔티티 직접 반환 금지(DTO 필수), 예외는 `GlobalExceptionHandler` 단일화

**참조 문서 (정합성 대상)**: `/CLAUDE.md`(최우선), `docs/PRD.md`, `docs/ROADMAP.md`, `docs/API.md`

> **주의**: `docs/API.md` · `docs/SCHEMA.md` · `docs/DESIGN.md` · `docs/CHECKLIST.md`는 **아직 작성되지 않았다**(PRD 0장 참조).
> 존재하지 않는 문서와의 정합성은 검증 대상이 아니다. 대신 **"PRD가 그 문서에 위임한 내용이 합리적인가"** 와
> **"미작성이 어떤 Phase를 막는가"** 를 평가한다. 없는 문서를 근거로 불일치를 주장하지 말 것.
...
- [ ] Access Token 30분 / Refresh Token 14일 회전 구조인가?
- [ ] Refresh Token이 httpOnly 쿠키 전용이고 DB에 해시로 저장되는가?
- [ ] 로그아웃이 서버 호출로 Refresh Token을 폐기하는가?
- [ ] 소셜 로그인이 Google 전용인가? (Kakao 없음)
- [ ] 비밀번호 재설정이 1회용·30분·전체 세션 폐기·계정 열거 방지를 만족하는가?
- [ ] 삭제가 Soft Delete(`deleted_at`)인가? 물리 삭제 표현이 있는가?
- [ ] 모든 목록 조회에 페이지네이션이 적용되는가?
- [ ] 인증 리소스에 본인 소유권 검증이 명시되었는가?
- [ ] DB명이 `todolist_db`(소문자)인가? 패키지명이 `com.example.todoapp`인가?
- [ ] 소유권 위반이 403이 아니라 404로 응답하는가?
- [ ] Tiptap 본문에 서버·클라이언트 이중 정화가 모두 있는가?
- [ ] S3·파일 업로드가 포함되지 않았는가?
...
- **PRD ↔ API.md**: `docs/API.md`가 **미작성**이므로 직접 대조는 불가하다. PRD가 서버 계약에 위임한 내용(엔드포인트 경로, 응답 래핑, 에러 코드)이 나중에 작성 가능한 수준으로 충분한지만 평가한다.
- **PRD ↔ ROADMAP**: 모든 F-xx / NF-xx가 `docs/ROADMAP.md`의 **Phase 0~10** 중 하나에 매핑되는가? ROADMAP은 `F-11~F-23` 같은 **범위 표기**를 쓰므로 개별 ID grep으로 판단하지 말고 범위를 펼쳐서 대조할 것. Phase 간 의존 관계(인증 기반 → 도메인 기능) 위반은 없는가?
...
| 이메일 전용 로그인 | ✅/❌ | |
| 비밀번호 6자 규칙 | ✅/❌ | |
| Access 30분 / Refresh 14일 회전 | ✅/❌ | |
| Refresh httpOnly 쿠키 + DB 해시 | ✅/❌ | |
| 서버 로그아웃 | ✅/❌ | |
| 비밀번호 재설정 정책 | ✅/❌ | |
| Google 전용 소셜 로그인 | ✅/❌ | |
| Tiptap 이중 정화 | ✅/❌ | |
...
| 페이지네이션 필수 | ✅/❌ | |
| 소유권 검증 | ✅/❌ | |
| DB명 todolist_db / 패키지 com.example.todoapp | ✅/❌ | |
| 소유권 위반 404 응답 | ✅/❌ | |

## 🔗 문서 정합성 결과
- PRD ↔ API.md(미작성): [위임 내용의 충분성 평가]
...
- [ ] PRD ↔ ROADMAP ↔ CLAUDE 충돌을 확인했는가? (API.md는 미작성이므로 대조 제외)
...
특히 프로젝트 불변 규칙 준수와 PRD/ROADMAP/CLAUDE 정합성을 반드시 확인한 뒤 종합 판정을 내려주세요.
```
