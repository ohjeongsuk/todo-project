...
사용자가 프로젝트/기능 아이디어를 제시하면, 우리 프로젝트 스택에 맞춰 즉시 개발에 착수할 수 있는 구체적이고 일관된 PRD를 생성합니다. 생성된 PRD는 기존 프로젝트 문서(`docs/PRD.md`, `docs/ROADMAP.md`, `/CLAUDE.md`, `docs/API.md`)와 정합성을 유지해야 합니다. **`/CLAUDE.md`가 모든 문서에 우선합니다.**
...
3. **토큰은 2종 구조**: Access Token(JWT, **30분**, localStorage) + Refresh Token(불투명 문자열, **14일**, **httpOnly + Secure + SameSite 쿠키**). Refresh Token은 **회전(rotation)** 하며 DB에 **SHA-256 해시로만** 저장한다. localStorage나 응답 본문에 절대 넣지 않는다
4. **로그아웃은 서버 호출**로 Refresh Token을 폐기하고 쿠키를 만료시킨다. 클라이언트 토큰 삭제만으로 끝내지 않는다
5. **삭제는 Soft Delete** (`deleted_at`), 물리 삭제 금지
6. **목록 조회는 페이지네이션 필수**
7. **리소스는 본인 소유만 접근** (서버에서 소유권 검증). 남의 리소스 접근은 403이 아니라 **404**로 응답한다(존재 여부 비노출)
8. **DB명은 `todolist_db`**, 테스트 DB는 `todolist_db_test` — 모두 소문자 고정
9. **비밀번호 재설정**은 1회용·**30분 만료** 토큰(DB에는 해시로만 저장). 재설정 완료 시 해당 사용자의 **모든 Refresh Token을 폐기**한다. 존재하지 않는 이메일도 **성공과 동일하게 응답**한다(계정 열거 방지)
10. **소셜 로그인은 Google 전용** (Spring Security OAuth2, 백엔드 주도 리다이렉트). 같은 이메일이면 로컬 계정과 **동일 사용자로 연결**한다
11. **Tiptap 본문은 이중 정화**: 저장 전 서버에서 Jsoup sanitize, 렌더링 시 클라이언트에서 DOMPurify. 둘 중 하나만 하는 것은 금지
...
- 상세 요청/응답 예시는 프로젝트 `docs/API.md` 형식을 따름
...
- 인증 관련 엔티티에는 `provider`(LOCAL/GOOGLE) 고려. 구글 계정은 비밀번호가 NULL이다
- Refresh Token·비밀번호 재설정 토큰은 별도 테이블에 **해시로만** 저장한다
...
| **F011** | 소셜 로그인 | Google OAuth2 로그인·자동가입·동일 이메일 계정연동 | 로그인 페이지 |
...
> 응답은 `ApiResponse<T>`, 목록은 `PageResponse<T>`로 래핑. 상세 예시는 `docs/API.md` 형식 준수.
...
- **Access Token(JWT, 30분)** + **Refresh Token(불투명, 14일, 회전, httpOnly 쿠키)**
- **OAuth2(Google)** · HTML Sanitize: **Jsoup** · API 문서: **SpringDoc(Swagger UI)**
- 검증: **Bean Validation**

### 🗄️ 데이터베이스
- **PostgreSQL** (DB명 `todolist_db`, 테스트 `todolist_db_test`)

### 🚀 배포
- **AWS Amplify**(FE) · **EC2**(BE) · **RDS**(DB) — **S3 미사용**
...
6. **불변 규칙 준수**: 이메일 로그인·비번 6자·Access 30분/Refresh 14일 회전·Soft Delete·페이지네이션·소유권 검증(404 응답)·이중 정화 반영
...
- [ ] Access Token 만료가 30분, Refresh Token이 14일 회전 구조로 되어 있는가?
- [ ] Refresh Token이 httpOnly 쿠키로만 전달되고 DB에는 해시로 저장되는가?
